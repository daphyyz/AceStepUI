...
router.post('/select-model', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      ...
      res.status(501).json({
        ...
        hint: 'Check ACE-Step Gradio availability and try again.',
      });
    }
  } catch (error) {
    console.error('Select model error:', error);
    res.status(500).json({ error: (error as Error).message || 'Model select failed' });
  }
});
